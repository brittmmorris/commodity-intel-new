...
export default function handler(req, res) {
  ...
  const locData = mockData[location]?.[year];

  if (!locData) {
    // Return safe fallback for unknown locations or years
    return res.status(200).json({
      commodities: [],
      summary: `We don't have data for "${location}" in ${year}.`,
      source: 'Frosty Mock Database',
    });
  }
